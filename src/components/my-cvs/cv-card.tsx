...
export function CVCard({ title, status, lastEdited, rating, previewStyle = 'orange' }: CVCardProps) {
  return (
    <article className='group h-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-lg'>
      <div className={`relative h-56 w-full ${previewClasses[previewStyle]}`}>
        ...
        <button
          type='button'
          className='absolute top-3 right-3 inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/85 text-slate-600 opacity-0 shadow-sm transition-opacity group-hover:opacity-100 hover:bg-white'
